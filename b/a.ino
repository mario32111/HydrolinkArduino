...
void mqttPub(String mensaje) {
  ...
  // Configurar el tema
  Serial.println("Enviando tema...");
  respuesta = enviarComandoAT("AT+CMQTTTOPIC=0," + String(topic.length()), 5000);
  Serial.println("Respuesta: " + respuesta);
  if (respuesta.indexOf(">") != -1) {
    SIM7670.print(topic);
    respuesta = enviarComandoAT("", 2000);
    Serial.println("Tema enviado, respuesta: " + respuesta);
  } else {
    Serial.println("Error al configurar el tema");
    return;
  }

  // Configurar el mensaje (payload)
  Serial.println("Enviando payload...");
  respuesta = enviarComandoAT("AT+CMQTTPAYLOAD=0," + String(mensaje.length()), 5000);
  Serial.println("Respuesta: " + respuesta);
  if (respuesta.indexOf(">") != -1) {
    SIM7670.print(mensaje);
    respuesta = enviarComandoAT("", 2000);
    Serial.println("Payload enviado, respuesta: " + respuesta);
  } else {
    Serial.println("Error al configurar el payload");
    return;
  }
